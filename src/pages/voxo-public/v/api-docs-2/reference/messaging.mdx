...
## List conversations {{ tag: 'GET', label: 'https://api.voxo.co/v2/messaging/conversations/:userId' }}
...
<Row>
    ...
    <Col sticky>
        <CodeGroup title="Request" tag="GET" label="/messaging/conversations/:userId">
            ```bash {{ title: 'cURL' }}
            curl --location -g --request GET 'https://api.voxo.co/v2/messaging/conversations/:userId' \
            --header 'Content-Type: application/json' \
            --header 'Authorization: Bearer {accessToken}'
            ```
        </CodeGroup>
